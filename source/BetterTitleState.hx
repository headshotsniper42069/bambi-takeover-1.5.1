...
class BetterTitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
        ...
        lime.app.Application.current.window.width = 640;
        lime.app.Application.current.window.height = 360;
		...
		lime.app.Application.current.window.onFullscreen.add(function(){
			lime.app.Application.current.window.fullscreen = false;
			lime.app.Application.current.window.onFullscreen.cancel();
		});
		...
		var endingscene:FlxSprite = new FlxSprite();
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			FlxG.sound.playMusic(Paths.music('betterMenu'), 0);
	...
	{
		var fullText:String = Assets.getText(Paths.txt('introTextBetter'));
	...
	override function update(elapsed:Float)
	{
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			FlxG.sound.play(Paths.sound('confirmMenuBetter'), 0.7);
	...
	override function beatHit()
	{
        ...
        if (curBeat >= 3)
        {
            ...
            funnyicon = Image.fromFile(Sys.getCwd() + "iconBetter.png");
            lime.app.Application.current.window.setIcon(funnyicon);
            lime.app.Application.current.window.title = "Friday Night Funkin | VS DAVE";
        }
